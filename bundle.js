(function () {
	...
	emitter$6.EventEmitter = EventEmitter;
	emitter$6.mixin = mixin;
	...
	function ShareDBError$3(code, message) {
	  ...
	  if (Error.captureStackTrace) {
	    Error.captureStackTrace(this, ShareDBError$3);
	  } else {
	    this.stack = new Error().stack;
	  }
	}

	ShareDBError$3.prototype = Object.create(Error.prototype);
	ShareDBError$3.prototype.constructor = ShareDBError$3;
	ShareDBError$3.prototype.name = 'ShareDBError';

	ShareDBError$3.CODES = {
	...
	*/
	...
	json.checkValidOp = function(op) {
	  for (var i = 0; i < op.length; i++) {
	    if (!isArray$1(op[i].p)) throw new Error('Missing path');
	  }
	};
	  ...
	  if (common != null) {
	    ...
	    if (oc.t && subtypes[oc.t]) {
	      if (c.t && c.t === oc.t) {
	        ...
	        } else if (!isArray$1(res) || res.length > 0) {
	          c.o = res;
	          json.append(dest, c);
	        }
	...
	exports.nextTick = function(callback) {
	  if (typeof browser$1 !== 'undefined' && browser$1.nextTick) {
	    return browser$1.nextTick.apply(null, arguments);
	  }
	...
	var require$$9 = /*@__PURE__*/getAugmentedNamespace(util$9);
	...
	var ERROR_CODE$2 = ShareDBError$2.CODES;
	...
	var doc = Doc$1;
	function Doc$1(connection, collection, id) {
	  emitter$5.EventEmitter.call(this);
	...
	emitter$5.mixin(Doc$1);

	Doc$1.prototype.destroy = function(callback) {
	  var doc = this;
	...
	Doc$1.prototype._setType = function(newType) {
	  if (typeof newType === 'string') {
	    newType = types$1.map[newType];
	  }
	  ...
	  if (newType) {
	    ...
	    var err = new ShareDBError$2(ERROR_CODE$2.ERR_DOC_TYPE_NOT_RECOGNIZED, 'Missing type ' + newType);
	...
	// @param snapshot.type
	// @param callback
	Doc$1.prototype.ingestSnapshot = function(snapshot, callback) {
	  if (!snapshot) return callback && callback();

	  if (typeof snapshot.v !== 'number') {
	    var err = new ShareDBError$2(
	      ERROR_CODE$2.ERR_INGESTED_SNAPSHOT_HAS_NO_VERSION,
	  ...
	  if (this.type || this.hasWritePending()) {
	    ...
	    if (this.version == null) {
	      ...
	      var err = new ShareDBError$2(
	        ERROR_CODE$2.ERR_DOC_MISSING_VERSION,
	  ...
	  var type = (snapshot.type === undefined) ? types$1.defaultType : snapshot.type;
	  this._setType(type);
	...
	Doc$1.prototype.whenNothingPending = function(callback) {
	  var doc = this;
	  util$8.nextTick(function() {
	    if (doc.hasPending()) {
	...
	Doc$1.prototype.hasPending = function() {
	  return !!(
	    this.inflightOp ||
	...
	Doc$1.prototype.hasWritePending = function() {
	  return !!(this.inflightOp || this.pendingOps.length);
	};

	Doc$1.prototype._emitNothingPending = function() {
	...
	Doc$1.prototype._emitResponseError = function(err, callback) {
	  if (err && err.code === ERROR_CODE$2.ERR_SNAPSHOT_READ_SILENT_REJECTION) {
	    this.wantSubscribe = false;
	    if (callback) {
	      callback();
	    }
	...
	Doc$1.prototype._handleFetch = function(error, snapshot) {
	  ...
	  if (callbacks.length) {
	    callback = function(error) {
	      util$8.callEach(callbacks, error);
	    };
	  }
	...
	Doc$1.prototype._handleSubscribe = function(error, snapshot) {
	...
	Doc$1.prototype._handleOp = function(err, message) {
	  if (err) {
	    if (this.inflightOp) {
	      // The server has rejected submission of the current operation. If we get
	      // an "Op submit rejected" error, this was done intentionally
	      // and we should roll back but not return an error to the user.
	      if (err.code === ERROR_CODE$2.ERR_OP_SUBMIT_REJECTED) err = null;
	...
	Doc$1.prototype._onConnectionStateChanged = function() {
	...
	Doc$1.prototype._resubscribe = function() {
	...
	Doc$1.prototype.fetch = function(callback) {
	  if (this.connection.canSend) {
	...
	Doc$1.prototype.subscribe = function(callback) {
	...
	// receiving updates
	Doc$1.prototype.unsubscribe = function(callback) {
	  var wantSubscribe = false;
	  this._queueSubscribe(wantSubscribe, callback);
	};

	Doc$1.prototype._queueSubscribe = function(wantSubscribe, callback) {
	...
	Doc$1.prototype._flushSubscribe = function() {
	...
	function combineCallbacks(callbacks) {
	  callbacks = callbacks.filter(util$8.truthy);
	...
	Doc$1.prototype.flush = function() {
	...
	function transformX(client, server) {
	  ...
	  if (server.del) {
	    return new ShareDBError$2(ERROR_CODE$2.ERR_DOC_WAS_DELETED, 'Document was deleted');
	  }
	  if (server.create) {
	    return new ShareDBError$2(ERROR_CODE$2.ERR_DOC_ALREADY_CREATED, 'Document already created');
	  }
	 ...
	 * @private
	 */
	Doc$1.prototype._otApply = function(op, source) {
	  if ('op' in op) {
	    if (!this.type) {
	      // Throw here, because all usage of _otApply should be wrapped with a try/catch
	      throw new ShareDBError$2(
	        ERROR_CODE$2.ERR_DOC_DOES_NOT_EXIST,
	    ...
	    if (!source && this.type === types$1.defaultType && op.op.length > 1) {
	...
	Doc$1.prototype._sendOp = function() {
	  ...
	  if (!op) {
	    var err = new ShareDBError$2(ERROR_CODE$2.ERR_INFLIGHT_OP_MISSING, 'No op to send on call to _sendOp');
	    return this.emit('error', err);
	  }
	  ...
	  if (op.seq == null) {
	    if (this.connection.seq >= util$8.MAX_SAFE_INTEGER) {
	      return this.emit('error', new ShareDBError$2(
	        ERROR_CODE$2.ERR_CONNECTION_SEQ_INTEGER_OVERFLOW,
	...
	Doc$1.prototype._submit = function(op, source, callback) {
	...
	Doc$1.prototype._pushOp = function(op, source, callback) {
	  op.source = source;
	  if (this.applyStack) {
	...
	Doc$1.prototype._popApplyStack = function(to) {
	  if (to > 0) {
	...
	Doc$1.prototype._tryCompose = function(op) {
	...
	Doc$1.prototype.submitOp = function(component, options, callback) {
	...
	Doc$1.prototype.create = function(data, type, options, callback) {
	  ...
	  if (!type) {
	    type = types$1.defaultType.uri;
	  }
	  if (this.type) {
	    var err = new ShareDBError$2(ERROR_CODE$2.ERR_DOC_ALREADY_CREATED, 'Document already exists');
	    if (callback) return callback(err);
	    return this.emit('error', err);
	  }
	...
	// @param options  {source: ...}
	// @param callback  called when operation submitted
	Doc$1.prototype.del = function(options, callback) {
	  if (typeof options === 'function') {
	    callback = options;
	    options = null;
	  }
	  if (!this.type) {
	    var err = new ShareDBError$2(ERROR_CODE$2.ERR_DOC_DOES_NOT_EXIST, 'Document does not exist');
	    if (callback) return callback(err);
	    return this.emit('error', err);
	  }
	  var op = {del: true};
	...
	Doc$1.prototype.pause = function() {
	  this.paused = true;
	};

	// Continue sending operations to the server
	Doc$1.prototype.resume = function() {
	  this.paused = false;
	  this.flush();
	};
	...
	Doc$1.prototype._opAcknowledged = function(message) {
	  if (this.inflightOp.create) {
	    this.version = message.v;
	  } else if (message.v !== this.version) {
	    // We should already be at the same version, because the server should
	    // have sent all the ops that have happened before acknowledging our op
	    logger$1.warn('Invalid version from server. Expected: ' + this.version + ' Received: ' + message.v, message);
	...
	Doc$1.prototype._rollback = function(err) {
	...
	Doc$1.prototype._hardRollback = function(err) {
	  ...
	  this.fetch(function() {
	    ...
	    for (var i = 0; i < pendingOps.length; i++) {
	      allOpsHadCallbacks = util$8.callEach(pendingOps[i].callbacks, err) && allOpsHadCallbacks;
	    }
	    if (err && !allOpsHadCallbacks) return doc.emit('error', err);
	  });
	};

	Doc$1.prototype._clearInflightOp = function(err) {
	  var inflightOp = this.inflightOp;

	  this.inflightOp = null;

	  var called = util$8.callEach(inflightOp.callbacks, err);
	...
	// a set of conditions.
	var query = Query$1;
	function Query$1(action, connection, id, collection, query, options, callback) {
	  emitter$4.EventEmitter.call(this);
	...
	emitter$4.mixin(Query$1);

	Query$1.prototype.hasPending = function() {
	  return !this.ready;
	};
	...
	Query$1.prototype.send = function() {
	  if (!this.connection.canSend) return;
	...
	Query$1.prototype.destroy = function(callback) {
	  ...
	  if (callback) util$7.nextTick(callback);
	};

	Query$1.prototype._onConnectionStateChanged = function() {
	...
	Query$1.prototype._handleFetch = function(err, data, extra) {
	  // Once a fetch query gets its data, it is destroyed.
	  this.connection._destroyQuery(this);
	  this._handleResponse(err, data, extra);
	};

	Query$1.prototype._handleSubscribe = function(err, data, extra) {
	  this._handleResponse(err, data, extra);
	};

	Query$1.prototype._handleResponse = function(err, data, extra) {
	...
	Query$1.prototype._ingestSnapshots = function(snapshots, finish) {
	  var results = [];
	...
	Query$1.prototype._finishResponse = function(err, callback) {
	  this.emit('ready');
	  this.ready = true;
	  if (err) {
	    this.connection._destroyQuery(this);
	...
	Query$1.prototype._handleError = function(err) {
	  this.emit('error', err);
	};

	Query$1.prototype._handleDiff = function(diff) {
	...
	Query$1.prototype._handleExtra = function(extra) {
	  this.extra = extra;
	  this.emit('extra', extra);
	};

	var emitter$3 = emitter$6;
	var util$6 = require$$9;

	var localPresence = LocalPresence$2;
	function LocalPresence$2(presence, presenceId) {
	  emitter$3.EventEmitter.call(this);
	...
	emitter$3.mixin(LocalPresence$2);

	LocalPresence$2.prototype.submit = function(value, callback) {
	  this.value = value;
	  this.send(callback);
	};

	LocalPresence$2.prototype.send = function(callback) {
	...
	LocalPresence$2.prototype.destroy = function(callback) {
	  var presence = this;
	...
	LocalPresence$2.prototype._sendPending = function() {
	  if (!this.connection.canSend) return;
	  var presence = this;
	...
	LocalPresence$2.prototype._ack = function(error, presenceVersion) {
	  var callback = this._getCallback(presenceVersion);
	  this._callbackOrEmit(error, callback);
	};

	LocalPresence$2.prototype._message = function() {
	  return {
	    a: 'p',
	...
	LocalPresence$2.prototype._getCallback = function(presenceVersion) {
	  var callback = this._callbacksByPresenceVersion[presenceVersion];
	  delete this._callbacksByPresenceVersion[presenceVersion];
	  return callback;
	};

	LocalPresence$2.prototype._callbackOrEmit = function(error, callback) {
	  if (callback) return util$6.nextTick(callback, error);
	  if (error) this.emit('error', error);
	};

	var util$5 = require$$9;

	var remotePresence = RemotePresence$2;
	function RemotePresence$2(presence, presenceId) {
	  this.presence = presence;
	  this.presenceId = presenceId;
	...
	RemotePresence$2.prototype.receiveUpdate = function(message) {
	...
	RemotePresence$2.prototype.destroy = function(callback) {
	  delete this.presence._remotePresenceInstances[this.presenceId];
	  delete this.presence.remotePresences[this.presenceId];
	  if (callback) util$5.nextTick(callback);
	};
	...
	var require$$4 = /*@__PURE__*/getAugmentedNamespace(async$1);

	var hat$2 = {exports: {}};

	var hat$1 = hat$2.exports = function (bits, base) {
	    if (!base) base = 16;
	    ...
	    if (parsed !== Infinity && parsed >= Math.pow(2, bits)) {
	        return hat$1(bits, base)
	    }
	    else return res;
	};

	hat$1.rack = function (bits, base, expandBy) {
	...
	var emitter$2 = emitter$6;
	var LocalPresence$1 = localPresence;
	var RemotePresence$1 = remotePresence;
	var util$4 = require$$9;
	var async = require$$4;
	var hat = hat$2.exports;

	var presence = Presence$2;
	function Presence$2(connection, channel) {
	  emitter$2.EventEmitter.call(this);
	...
	emitter$2.mixin(Presence$2);

	Presence$2.prototype.subscribe = function(callback) {
	  this._sendSubscriptionAction(true, callback);
	};

	Presence$2.prototype.unsubscribe = function(callback) {
	  this._sendSubscriptionAction(false, callback);
	};

	Presence$2.prototype.create = function(id) {
	  id = id || hat();
	...
	Presence$2.prototype.destroy = function(callback) {
	  var presence = this;
	...
	Presence$2.prototype._sendSubscriptionAction = function(wantSubscribe, callback) {
	...
	Presence$2.prototype._handleSubscribe = function(error, seq) {
	  if (this.wantSubscribe) this.subscribed = true;
	  var callback = this._subscriptionCallback(seq);
	  this._callbackOrEmit(error, callback);
	};

	Presence$2.prototype._handleUnsubscribe = function(error, seq) {
	  this.subscribed = false;
	  var callback = this._subscriptionCallback(seq);
	  this._callbackOrEmit(error, callback);
	};

	Presence$2.prototype._receiveUpdate = function(error, message) {
	  var localPresence = util$4.dig(this.localPresences, message.id);
	  if (localPresence) return localPresence._ack(error, message.pv);

	  if (error) return this.emit('error', error);
	  var presence = this;
	  var remotePresence = util$4.digOrCreate(this._remotePresenceInstances, message.id, function() {
	...
	Presence$2.prototype._updateRemotePresence = function(remotePresence) {
	...
	Presence$2.prototype._broadcastAllLocalPresence = function(error) {
	  if (error) return this.emit('error', error);
	...
	Presence$2.prototype._removeRemotePresence = function(id) {
	  this._remotePresenceInstances[id].destroy();
	  delete this._remotePresenceInstances[id];
	  delete this.remotePresences[id];
	};

	Presence$2.prototype._onConnectionStateChanged = function() {
	  if (!this.connection.canSend) return;
	  this._resubscribe();
	  for (var id in this.localPresences) {
	...
	Presence$2.prototype._resubscribe = function() {
	  var callbacks = [];
	...
	Presence$2.prototype._subscriptionCallback = function(seq) {
	  var callback = this._subscriptionCallbacksBySeq[seq];
	  delete this._subscriptionCallbacksBySeq[seq];
	  return callback;
	};

	Presence$2.prototype._callbackOrEmit = function(error, callback) {
	  if (callback) return util$4.nextTick(callback, error);
	  if (error) this.emit('error', error);
	};

	Presence$2.prototype._createLocalPresence = function(id) {
	  return new LocalPresence$1(this, id);
	};

	Presence$2.prototype._createRemotePresence = function(id) {
	  return new RemotePresence$1(this, id);
	};

	Presence$2.prototype._callEachOrEmit = function(callbacks, error) {
	  var called = util$4.callEach(callbacks, error);
	  if (!called && error) this.emit('error', error);
	};

	var LocalPresence = localPresence;
	var ShareDBError$1 = error;
	var util$3 = require$$9;
	var ERROR_CODE$1 = ShareDBError$1.CODES;

	var localDocPresence = LocalDocPresence$1;
	function LocalDocPresence$1(presence, presenceId) {
	...
	LocalDocPresence$1.prototype = Object.create(LocalPresence.prototype);

	LocalDocPresence$1.prototype.submit = function(value, callback) {
	  if (!this._doc.type) {
	...
	LocalDocPresence$1.prototype.destroy = function(callback) {
	...
	LocalDocPresence$1.prototype._sendPending = function() {
	...
	LocalDocPresence$1.prototype._registerWithDoc = function() {
	...
	LocalDocPresence$1.prototype._transformAgainstOp = function(op, source) {
	  var presence = this;
	  this._pendingMessages.forEach(function(message) {
	    try {
	...
	LocalDocPresence$1.prototype._handleCreateOrDel = function() {
	  this._pendingMessages.forEach(function(message) {
	    message.p = null;
	  });
	...
	LocalDocPresence$1.prototype._handleLoad = function() {
	  this.value = null;
	  this._pendingMessages = [];
	};

	LocalDocPresence$1.prototype._message = function() {
	...
	LocalDocPresence$1.prototype._transformPresence = function(value, op, source) {
	  var type = this._doc.type;
	  if (!util$3.supportsPresence(type)) {
	    throw new ShareDBError$1(
	      ERROR_CODE$1.ERR_TYPE_DOES_NOT_SUPPORT_PRESENCE,
	...
	var remoteDocPresence = RemoteDocPresence$1;
	function RemoteDocPresence$1(presence, presenceId) {
	...
	RemoteDocPresence$1.prototype = Object.create(RemotePresence.prototype);

	RemoteDocPresence$1.prototype.receiveUpdate = function(message) {
	...
	RemoteDocPresence$1.prototype.destroy = function(callback) {
	  this._doc.removeListener('op', this._opHandler);
	...
	RemoteDocPresence$1.prototype._registerWithDoc = function() {
	  this._doc.on('op', this._opHandler);
	...
	RemoteDocPresence$1.prototype._setPendingPresence = function() {
	...
	RemoteDocPresence$1.prototype._handleOp = function(op, source, connectionId) {
	...
	RemoteDocPresence$1.prototype._transformAgainstOp = function(op, isOwnOp) {
	...
	RemoteDocPresence$1.prototype._catchUpStalePresence = function() {
	...
	RemoteDocPresence$1.prototype._startCachingOps = function() {
	  this._opCache = [];
	};

	RemoteDocPresence$1.prototype._stopCachingOps = function() {
	  this._opCache = null;
	};

	RemoteDocPresence$1.prototype._cacheOp = function(op, isOwnOp) {
	  if (this._opCache) {
	    op = op ? {op: op} : null;
	...
	var Presence$1 = presence;
	var LocalDocPresence = localDocPresence;
	var RemoteDocPresence = remoteDocPresence;

	function DocPresence$1(connection, collection, id) {
	  var channel = DocPresence$1.channel(collection, id);
	  Presence$1.call(this, connection, channel);

	  this.collection = collection;
	  this.id = id;
	}
	var docPresence = DocPresence$1;

	DocPresence$1.prototype = Object.create(Presence$1.prototype);

	DocPresence$1.channel = function(collection, id) {
	  return collection + '.' + id;
	};

	DocPresence$1.prototype._createLocalPresence = function(id) {
	  return new LocalDocPresence(this, id);
	};

	DocPresence$1.prototype._createRemotePresence = function(id) {
	  return new RemoteDocPresence(this, id);
	};

	var snapshot = Snapshot$1;
	function Snapshot$1(id, version, type, data, meta) {
	  this.id = id;
	  this.v = version;
	  this.type = type;
	  this.data = data;
	...
	var emitter$1 = emitter$6;

	var snapshotRequest = SnapshotRequest$2;

	function SnapshotRequest$2(connection, requestId, collection, id, callback) {
	  emitter$1.EventEmitter.call(this);
	...
	emitter$1.mixin(SnapshotRequest$2);

	SnapshotRequest$2.prototype.send = function() {
	  if (!this.connection.canSend) {
	    return;
	  }
	...
	SnapshotRequest$2.prototype._onConnectionStateChanged = function() {
	  if (this.connection.canSend) {
	...
	SnapshotRequest$2.prototype._handleResponse = function(error, message) {
	  this.emit('ready');
	...
	var SnapshotRequest$1 = snapshotRequest;
	var util$2 = require$$9;

	var snapshotVersionRequest = SnapshotVersionRequest$1;

	function SnapshotVersionRequest$1(connection, requestId, collection, id, version, callback) {
	  SnapshotRequest$1.call(this, connection, requestId, collection, id, callback);

	  if (!util$2.isValidVersion(version)) {
	...
	SnapshotVersionRequest$1.prototype = Object.create(SnapshotRequest$1.prototype);

	SnapshotVersionRequest$1.prototype._message = function() {
	  return {
	    a: 'nf',
	    id: this.requestId,
	    c: this.collection,
	...
	var snapshotTimestampRequest = SnapshotTimestampRequest$1;

	function SnapshotTimestampRequest$1(connection, requestId, collection, id, timestamp, callback) {
	  SnapshotRequest.call(this, connection, requestId, collection, id, callback);

	  if (!util$1.isValidTimestamp(timestamp)) {
	...
	SnapshotTimestampRequest$1.prototype = Object.create(SnapshotRequest.prototype);

	SnapshotTimestampRequest$1.prototype._message = function() {
	  return {
	    a: 'nt',
	    id: this.requestId,
	    c: this.collection,
	...
	client.Error = error;
	client.Query = query;
	client.types = types$2;
	client.logger = logger_1;
